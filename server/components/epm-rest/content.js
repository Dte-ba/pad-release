...
module.exports = function (router) {
  ...
  router.get('/content/:repository/:uid', function (req, res, next) {
    ...
    manager.get(rname).progress(function (info) {}).fail(function (err) {
      ...
      repo.findOne({ uid: uid }, function (err, pkg) {
        if (err) {
          return next(err);
        }
        var info = {
        ...
        repo.engine.content(repo, info, pkg).fail(function (err) {
          next(err);
        }).done(function (data) {
  ...
  function writeResolved(info, res, rname, statusCode) {
    ...
    if (info.files.length === 1) {
      ...
      var bpath = extractBasepath(info.files.map(function (f) {
        return f.filename;
      }));
    ...
    function extractBasepath(entries) {
      ...
      var haveFunc = function haveFunc(sd) {
        return sd.length > idx;
      };

      var eqFunc = function eqFunc(i) {
        return curr === i;
      };

      var eqmapFunc = function eqmapFunc(a) {
        return a[idx];
      };
